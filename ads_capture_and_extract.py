# ads_capture_and_extract.py – iframes + smart bildval + inbäddade bilder, ingen OCR
...
# Hur många annonser får nedladdad + inbäddad bild
MAX_IMAGE_ADS = int(os.getenv("MAX_IMAGE_ADS", "150"))

DOWNLOAD_IMAGES = os.getenv("DOWNLOAD_IMAGES", "1") not in ("0", "false", "False")


def set_paths(base_dir):
    """Repoint global paths in i run-katalogen."""
...
# ============================================================
# EXCEL – text + Bild-URL + inbäddade bilder för första MAX_IMAGE_ADS
# ============================================================
...
def process_candidates_and_save(run_dir):
    ...
    for idx, ad in enumerate(ads, start=1):
        ...
        # Bara de första MAX_IMAGE_ADS annonserna får nerladdad + inbäddad bild
        do_image = (idx <= MAX_IMAGE_ADS)

        if do_image and img_url and DOWNLOAD_IMAGES:
            try:
                url = img_url
                if url.startswith("//"):
                    url = "https:" + url
        ...
        # bygg enkel beskrivning från text-rader
        ...
        rows.append({
            ...
            "Text": ad.get("text", ""),
            "Bild-URL": img_url,
            "Bildfil": img_file,   # tom för rader > MAX_IMAGE_ADS
        })
    ...
    # Bädda in bilder – bara rader som har Bildfil (dvs första MAX_IMAGE_ADS)
